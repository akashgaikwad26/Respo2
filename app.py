...
REPORT_MODEL_MODE = os.getenv("RESPIGUARD_REPORT_MODEL", "auto").lower()
USE_MEDGEMMA_REPORT = REPORT_MODEL_MODE == "medgemma" or (
    REPORT_MODEL_MODE == "auto" and DEVICE != "cpu"
)
TARGET_SR = 16000
TARGET_LEN = 32000
MIN_RMS = 0.008
...
def generate_medgemma_report(risk_score):
    ...
    if tokenizer is None or llm_model is None:
        return normalize_report_payload(build_fallback_report(risk_level), risk_level)
...
with col_input:
    ...
    input_mode = st.radio(
        "Audio Input Method",
        ["Record directly", "Upload audio file"],
        horizontal=True,
    )
    if input_mode == "Record directly":
        audio_source = st.audio_input("Input Stream")
    else:
